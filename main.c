...
double calc_rest_area(Room *list) {
  ...
  if (list->super->bg_angle == -1)
    return 0;

  dangle = (list->super->bg_angle + 360) -
...
void algorithm(Room *list) {
  ...
  if (area > 1e-8) {
    rooms_qnt =
        find_rooms_below_max(list); // encontra comodos fora da area maxima
    printf("rooms qnt: %d\n", rooms_qnt);
    if (rooms_qnt > 0) {
    ...
    } else // rooms_qnt == 0 -> nao eh possivel aumentar os quartos
    {
      ...
      if (!ret || area > 1e-8)
        propagate_without_limit(list, area); // força um aumento
      return;
    }
  } else if (area < 0) {
    rooms_qnt = find_rooms_above_min(
        list); // encontra comodos acima do limite minimo de area
    printf("rooms qnt: %d\n", rooms_qnt);
    if (rooms_qnt > 0) {
      add_area = area / rooms_qnt;
      propagate_new_area(
          list, add_area); // propaga a fraçao negativa da area para os comodos
    }
    return;
  } else {
...
int main(int argc, char *argv[]) {
  ...
  if ((r2 < 6.2 || r2 > 8.2) || (r3 < 6.2 || width > 30))
    return printf(
        "Não é possível gerar uma casa circular com essas dimensões!\n");

  // Com base no raio_minimo/2 e no raio atual/2, sorteia o raio do comodo
  // central (estar/jantar)
  ...
  // se o angulo não estão nessa faixa, comodos serao invertidos a partir do
  // hall
